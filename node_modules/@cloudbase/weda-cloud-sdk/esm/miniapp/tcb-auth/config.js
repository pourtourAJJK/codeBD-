...
import { getConfig, useTcbApi, OauthClientStorgeBase, generateOauthClientRequest, getUrlPath } from '../../common';
import { EVENT_BUS } from '../../eventBus';
import { wxMpStorage } from '../../storage';
async function processBaseRequestOptions(options, transferUrl = false) {
    ...
    const processUrl = useTcbApi() || !transferUrl
        ? url
...
const AUTH_BASE_CONFIG = {
    storage: new OauthClientStorgeBase({ localStorage: wxMpStorage }),
    captchaOptions: {
        openURIWithCallback: (_url) => {
            ...
            return new Promise((resolve) => {
                ...
                EVENT_BUS.emit('CAPTCHA_DATA_CHANGE', Object.assign(Object.assign({}, restQueryObj), { token, url }));
                EVENT_BUS.once('RESOLVE_CAPTCHA_DATA', (res) => {
                    resolve(res);
                });
            });
        },
...
export function generateAuthBaseConfig(app) {
    return Object.assign({ baseRequest: generateOauthClientRequest(useTcbApi()
            ? async (options) => {
                // Objects must be copied to prevent modification of data such as body.
