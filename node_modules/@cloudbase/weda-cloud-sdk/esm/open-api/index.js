import { WxRequest, processBaseRequestOptions } from './wx';
/**
 * h5跳转地址带上微信小程序openid生成的授权登录token，用于小程序openid授权登录
 * @param src h5跳转地址
 * @returns
 */
export async function getUrlWithOpenidToken(src, envId, useWxCloud) {
    ...
    return new Promise((resolve) => {
        wx.login({
            success: async (res) => {
                ...
                try {
                    const { provider_token: providerToken } = await WxRequest('/auth/v1/provider/token', {
                        body: {
 ...
 * @returns
 */
export async function getUrlWithOpenidTokenWxCloud(src, envId) {
    ...
    try {
        const { miniProgram } = wx.getAccountInfoSync();
        const { url, method, headers, body } = await processBaseRequestOptions({
