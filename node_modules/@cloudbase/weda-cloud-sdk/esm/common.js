 ...
 */
export function getDataSourceViewId() {
    return;
}
/**
 * 更新配置 内部使用
 ...
 */
export function getDatasourceCloudFnName(dsKeyInfo) {
 ...
 */
export function getCommonCloudFnName(needNewFeature) {
...
/** 设置默认参数 */
export function setDefaultParams(dsName, params) {
 ...
 * @returns
 */
export function getDefaultParams(dsName, options) {
 ...
 * @returns
 */
export function createSetDefaultParams(dsName) {
    return (params) => {
...
export function setDatasetProfiles(obj) {
    setConfig({
        datasetProfiles: Object.assign(getConfig('datasetProfiles') || {}, obj),
    });
}
export function getDatasetProfiles(key) {
 ...
 * 同步修改
 */
export class OauthClientStorgeBase {
    constructor(config) {
 ...
 */
const URL_REG = /^[^:]+:\/\/[^/]+(\/[^?#]+)/;
export function generateOauthClientRequest(request) {
    if (!request) {
        return undefined;
    }
...
export async function _generatePrivatelinkAdapter(env, host) {
    if (env.privatelink) {
