 ...
 * 已成功获取的文件ID及访问链接 Map
 */
const successCache = new ttlcache_1.default({
...
function createTask(fileIds, resolve, reject) {
    ...
    let files = (Array.isArray(fileIds) ? fileIds : [fileIds]).filter(v => (0, common_1.isObject)(v) || (0, common_1.isString)(v));
    ...
    clearTimeout(tid);
    const now = Date.now();
    nextTime = (!nextTime || now > nextTime) ? now + config_1.cloudConfig.tempURLMaxWaitGap : nextTime;
    // @ts-ignore
...
async function doGetTempURL() {
    ...
    try {
        ...
        const allResponse = (await (0, common_1.PromiseAllSettled)(chunk(fileIds, MAX_BATCH_LIMIT)
...
function isFileItemSuccess(fileItem) {
    ...
    if ((0, common_1.hasOwn)(fileItem, 'code'))
        return fileItem.code === 'SUCCESS';
    // 小程序, status 0 表示成功
    if ((0, common_1.hasOwn)(fileItem, 'status'))
