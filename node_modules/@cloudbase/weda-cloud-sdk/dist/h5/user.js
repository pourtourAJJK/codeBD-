...
async function signIn(params) {
    ...
    const currentUser = await (0, cache_1.dealCache)({
        key: 'SIGNIN_INFO',
        type: 'get',
    });
    if (currentUser) {
    ...
    if (loginState.hasLogin) {
        ...
        if (res.redirect_uri) {
            try {
                ...
                console.log('[cloud-sdk] signOut redirect_uri error', error);
            }
        }
    }
    // 非匿名情况, 重新匿名登录
...
async function getUserInfo(force = false) {
    if (force || !observedInfo.currentUser) {
        const { app } = await (0, tcb_1.getTcbInstance)();
        const currentUser = await (0, cache_1.dealCache)({ key: 'USER_INFO', type: 'get' });
        if (currentUser) {
