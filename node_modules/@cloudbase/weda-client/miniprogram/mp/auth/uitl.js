...
export async function getAccessPermission(appId, packageName = '', pageId, isAdminPortal = false) {
    ...
    const pageRoute = packageName ? `packages/${packageName}/pages/${pageId}/index` : `pages/${pageId}/index`;
    try {
        const localCache = await dealCache({ key: 'RENDER_ACCESS', type: 'get' });
        if (localCache?.[cacheKey]) {
            // 异步更新
            getAccessPermissionRes({ isAdminPortal, cacheKey, appId, pageRoute });
    ...
    return getAccessPermissionRes({ isAdminPortal, cacheKey, appId, pageRoute });
}
async function getAccessPermissionRes({ isAdminPortal, cacheKey, appId, pageRoute }) {
    ...
    if (Array.isArray(res)) {
        ...
        // 当前页面和请求权限的页面一致才更新缓存
        if (!currentPage || currentPage.route === pageRoute) {
