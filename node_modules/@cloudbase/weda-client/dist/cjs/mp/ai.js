...
exports.createAiMethods = exports.createMpAiAPI = void 0;
...
function tcbApi() {
    return (0, weda_cloud_sdk_1.getConfig)('endpointType') === 'tcb-api';
}
async function getAI() {
    if (tcbApi()) {
        // 不能用 wx.cloud，要用 js sdk 构造一个和 wx.cloud.extend.AI 一样的对象。 ps：大致一样，onText 那些回调先不加
        const cloudbase = await weda_cloud_sdk_1.CLOUD_SDK.getCloudInstance();
        ...
        return (await weda_cloud_sdk_1.CLOUD_SDK.getCloudInstance())?.extend?.AI;
    }
}
...
function createMpAiAPI(ctx) {
    return { ai: createAiMethods(ctx) };
}
exports.createMpAiAPI = createMpAiAPI;
const DEFAULT_STREAM_TIMEOUT = 60 * 3 * 1000;
function createAiMethods(ctx) {
...
exports.createAiMethods = createAiMethods;
