...
exports.patchWdigetPropsWithEvtListeners = exports.processStaticResourceAttribute = exports.mergeDynamic2StaticData = exports.getMpEventHandlerName = exports.deepEqual = exports.throttle = exports.touchObj = exports.getDeep = exports.createEventHandlers = exports.generateDataContext = exports.createComputed = exports.toDash = exports.compLowcodes = void 0;
...
function createComputed(funcs, bindContext = null) {
    const computed = {};
...
function generateDataContext(widget) {
    const dataContext = {};
    while (widget?._scope) {
...
exports.generateDataContext = generateDataContext;
/**
 * @param evtListeners
 * @param [options]
 */
function createEventHandlers(evtListeners, options) {
    ...
    for (const name in evtListeners) {
        ...
        evtHandlers[name] = function (event = {}) {
            ...
            const target = !!event?.target?.id ? (0, widget_1.getWidget)(owner.widgets, event.target.id) : undefined;
            const currentTarget = !!event?.currentTarget?.id ? (0, widget_1.getWidget)(owner.widgets, event.currentTarget.id) : undefined;
            ...
            const forContext = (!!currentTarget && (0, widget_1.generateForContextOfWidget)(currentTarget)) || {};
            const dataContext = (0, mobx_1.untracked)(() => generateDataContext(currentTarget));
            const $w = (0, mobx_1.untracked)(() => (0, widget_1.generateWidgetAPIContext)(owner?.__internal__?.$w, currentTarget, forContext));
...
function mergeDynamic2StaticData(staticData, dataBinds, context, combainErrors = false) {
...
exports.mergeDynamic2StaticData = mergeDynamic2StaticData;
function replaceStaticResourceAttribute(obj, replacer, paths, index = 0) {
    const path = paths[index];
    const value = (0, vender_1.get)(obj, path);
    if (index === paths.length - 1) {
        try {
            if (value) {
                (0, vender_1.set)(obj, path, replacer(value));
            }
        }
        catch (e) {
            console.error(e);
...
function processStaticResourceAttribute(data, replacer, property) {
    const { app } = (0, app_1.getWedaAPI)();
...
exports.processStaticResourceAttribute = processStaticResourceAttribute;
function patchWdigetPropsWithEvtListeners(widgetProps, evtListeners) {
...
exports.patchWdigetPropsWithEvtListeners = patchWdigetPropsWithEvtListeners;
