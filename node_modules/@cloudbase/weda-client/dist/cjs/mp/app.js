...
function createMpApp(initData = {}) {
    ...
    const mpApp = Object.assign(apiWithUtils, {
        ...
        ...(0, ai_1.createMpAiAPI)(apiWithUtils),
        /**
         * @deprecated
         */
        invoke(params) {
         ...
         * @deprecated
         */
        _setStateVal(config, $w) {
            const [key, name] = config?.varPath?.split?.('.') || [];
            const whiteList = ['$global', '$page', '$comp'];
            if (!whiteList.includes(key)) {
                console.warn('[@deprecated] 禁止跨页面设置变量值，后期版本将放弃对此的支持');
            }
            if (whiteList.includes(key) && $w) {
                return (0, utils_1.patchDeprecatedSetStateVar)($w, config.varPath, config.val);
            }
            else {
                if (key === '$page') {
                    const $page = mpApp.utils.getCurrentPage();
