...
exports.createWebActionsAPI = void 0;
const scanCode_1 = require("./scanCode/scanCode");
 ...
 * @returns
 */
function createWebActionsAPI(app, sdks, options) {
    const actions = {};
    ...
    Object.keys(sdks).forEach((item) => {
        ...
        switch (item) {
            case constants_1.ACTIONS_KEY.showToast: {
                action = function (obj) {
            ...
            case constants_1.ACTIONS_KEY.showModal: {
            ...
            case constants_1.ACTIONS_KEY.scanCode: {
                action = (options) => {
                    ...
                    (0, scanCode_1.scanCode)(options);
                };
                break;
            }
            case constants_1.ACTIONS_KEY.openLocation: {
                action = () => { };
                break;
            }
        }
        // 只添加action相关的
        if (item in constants_1.ACTIONS_KEY) {
            actions[item] = processAction(item, action);
        }
    });
    actions[constants_1.ACTIONS_KEY.callProcess] = function ({ processKey, ProcessKey, startParams, StartParams }) {
        return (0, models_1.callWedaApiFn)('StartProcessWithParams', {
            ProcessKey: processKey || ProcessKey,
            TriggerType: 1,
            StartParams: startParams || StartParams || [],
        });
    };
    actions[constants_1.ACTIONS_KEY.callWorkflow] = function ({ flowId, params = {}, debugEnabled = false }) {
        return (0, models_1.callWedaApiFn)('StartFlowInstance', {
            FlowId: flowId,
            Params: params,
            DebugEnabled: debugEnabled,
        });
    };
    actions[constants_1.ACTIONS_KEY.callPhone] = function ({ tel }) {
        return (location.href = `tel:${tel}`);
    };
    actions[constants_1.ACTIONS_KEY.requestSubscribeMessage] = () => { };
    actions[constants_1.ACTIONS_KEY.reserveChannelsLive] = () => { };
    actions[constants_1.ACTIONS_KEY.openChannelsUserProfile] = () => { };
    actions[constants_1.ACTIONS_KEY.openChannelsLive] = () => { };
    actions[constants_1.ACTIONS_KEY.openChannelsEvent] = () => { };
    actions[constants_1.ACTIONS_KEY.openChannelsActivity] = () => { };
    actions[constants_1.ACTIONS_KEY.getChannelsLiveNoticeInfo] = () => { };
    actions[constants_1.ACTIONS_KEY.getChannelsLiveInfo] = () => { };
    actions[constants_1.ACTIONS_KEY.setClipboardData] = async ({ data, success = (res) => { }, fail = (err) => { }, complete = (err) => { }, }) => {
        let res = {};
        try {
            /**
             * 异步操作中 document 失焦
    ...
    actions[constants_1.ACTIONS_KEY.exportData] = async ({ data, fileName = 'download', fileType = 'csv', options }) => {
        const INVALID_PARAMS = 'InvalidParams';
        if (!Array.isArray(data) && !(0, vender_1.isPlainObject)(data)) {
        ...
        switch (fileType) {
            ...
            case 'xlsx': {
                const XLSX = await (process.env.compileTool === 'rollup'
                    ? require('xlsx')
                    : Promise.resolve().then(() => __importStar(require(/* webpackChunkName: "extra-package" */ 'xlsx'))));
    ...
    actions[constants_1.ACTIONS_KEY.getEnumValue] = (params) => {
        return (0, utils_1.getEnumValue)(params, app?.__internal__?.$w?.app);
    };
    actions[constants_1.ACTIONS_KEY.setTitle] = (newTitle) => new Promise((res, rej) => {
        try {
...
exports.createWebActionsAPI = createWebActionsAPI;
function s2ab(s) {
    let buf;
    let view;
...
async function getDataAsCSV(rowData) {
    ...
    const papa = await (process.env.compileTool === 'rollup'
        ? require('papaparse')
        : Promise.resolve().then(() => __importStar(require(/* webpackChunkName: "extra-package" */ 'papaparse'))));
    return papa.unparse(convertData);
}
function sanitizeTabularDataForExcel(e) {
    return e
        .map((e) => e.map((e) => ((0, vender_1.isObject)(e) ? JSON.stringify(e) : e)))
        .map((e) => e.map((e) => {
