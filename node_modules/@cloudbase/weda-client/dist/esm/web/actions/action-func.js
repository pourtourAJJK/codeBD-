import { getDocument, errorHandler, getParameterError, inlineStyle, calPxToREM } from './utils';
...
class Toast {
    ...
    constructor() {
        ...
        this.style = {
            ...
            pcToastStyle: {
                ...
                'line-height': calPxToREM(30),
                padding: `0 ${calPxToREM(18)}`,
                overflow: 'hidden',
                'white-space': 'nowrap',
    ...
    create(options, type = 'h5') {
        ...
        if (config.image) {
            this.icon.setAttribute('style', inlineStyle({ ...imageStyle, 'background-image': `url(${config.image})` }));
        }
        else {
            let iconStyle = config.icon === 'loading' ? (type === 'h5' ? loadingStyle : pcLoadingStyle) : successStyle;
            this.icon.setAttribute('style', inlineStyle({ ...iconStyle, ...(config.icon === 'none' ? { display: 'none' } : {}) }));
        ...
        this.title.setAttribute('style', inlineStyle(type === 'h5' ? textStyle : pcTextStyle));
...
export function showToast(options) {
    ...
    options._type = 'toast';
    // verify options
    let handler = errorHandler(options.fail, options.complete);
...
export function showLoading(options, type = 'h5') {
    if (options === void 0) {
        options = {};
    }
    init(getDocument());
...
sdk.hideToast = hideToast;
sdk.showLoading = showLoading;
sdk.hideLoading = hideLoading;
export const actionSdk = sdk;
